...
```{r}
# OI covariates dataset for control variables Chetty used

download.file("https://opportunityinsights.org/wp-content/uploads/2018/04/online_table4-2.xlsx",
              "raw-data/county_covariates.xlsx", mode="wb")

oi_cov <- read_xlsx("raw-data/county_covariates.xlsx", skip = 8) 

oi_cov <- oi_cov[-c(1,2,3, 4), ]

oi_cov <- oi_cov %>% clean_names()

oi_cov$county_fips_2000 <- as.numeric(oi_cov %>% pull(county_fips_2000))
oi_cov$state_fips <- as.numeric(oi_cov %>% pull(state_fips))

# I also made sure that the merged column was numeric to merge with other datasets later

oi_cov$countyid <- as.numeric(oi_cov %>% pull(county_fips_2000))
```
...
```{r}
# comparing how similar the measures of White-Black Relative Diversity and White-Hispanic Relative Diversity are, aka checking for collinearity

div_cor_plot <- seda_oi_cov %>% 
  filter(subgroup == "all") %>% 
  ggplot(aes(x=rswhtblk, y= rswhthsp, size= tot_asmts_ela)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(x = "Black-White Relative Diversity",
       y = "Hispanic-White Relative Diversity") +
  theme_bw() +
  guides(size = FALSE)

ggsave("div_cor_plot.png")

div_cor = lm(rswhtblk ~ rswhthsp, data = seda_oi_cov)
```
...
kfr_ses <- seda_oi_cov %>% 
  filter(subgroup == "all") %>% 
  ggplot(aes(x=sesavgall, y= kfr_pooled_pooled_p25)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(x = "SES Composite Score",
       y = "Intergenerational Mobility (P25)") +
  theme_bw()
...
ggsave("perecd_mob_Rplot.png")

perecd_25_wb <- seda_oi_cov %>% 
  filter(subgroup == "all") %>% 
  filter(perecd <= .25) %>% 
  ggplot(aes(y=mn_avg_mth_eb, x= rswhtblk)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "Mean Standardized Math Score",
       x = "Black-White Relative Diversity",
       cex.axis = 1.5) +
  theme_bw()
...
pov_wb_bubble <- seda_oi_cov %>% 
  filter(subgroup == "all") %>% 
  ggplot(aes(y=mn_avg_mth_eb, x= rswhtblk, size = povertyavgall)) +
  geom_point(alpha=.25) + 
...
pov_wb_race <- seda_oi_cov %>% 
  ...
  ggplot(aes(y=mn_avg_mth_eb, x= rswhtblk, color= subgroup)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "Mean Standardized Math Score",
       x = "Black-White Relative Diversity",
...
rsecdnec_test_by_race <- seda_oi_cov %>% 
  filter(subgroup == "asn" | subgroup == "blk" | subgroup == "hsp" | subgroup == "wht") %>% 
  ggplot(aes(y=mn_avg_mth_eb, x= rsecdnec, size = tot_asmts_mth)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "Mean Standardized Math Score",
       x = "ECD/Non-ECD Relative Diversity Index",
       cex.axis = 1.5) +
  theme_bw() +
  facet_wrap(~subgroup) +
   guides(size = FALSE)
...
rsecdnec_grd_by_race <- seda_oi_cov %>% 
  filter(subgroup == "asn" | subgroup == "blk" | subgroup == "hsp" | subgroup == "wht") %>% 
  ggplot(aes(y=mn_grd_mth_eb, x= rsecdnec, size = tot_asmts_mth)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "County Grade Slope of Mean Math Scores",
       x = "ECD/Non-ECD Relative Diversity Index",
       cex.axis = 1.5) +
  theme_bw() +
  facet_wrap(~subgroup) +
...
ggsave("rsecdnec_grd_by_race.png")

rsecdnec_coh_by_race <- seda_oi_cov %>% 
  filter(subgroup == "asn" | subgroup == "blk" | subgroup == "hsp" | subgroup == "wht") %>% 
  ggplot(aes(y=mn_coh_mth_eb, x= rsecdnec, size = tot_asmts_mth)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "County Cohort Slope of Mean Math Scores",
       x = "ECD/Non-ECD Relative Diversity Index",
       cex.axis = 1.5) +
  theme_bw() +
  facet_wrap(~subgroup) +
...
ggsave("rsecdnec_coh_by_race.png")

# seeing how concentration of school ECD's effect on test scores varies by race

perecd_race_plot <- seda_oi_cov %>% 
  filter(subgroup == "asn" | subgroup == "blk" | subgroup == "hsp" | subgroup == "wht") %>% 
  ggplot(aes(y=mn_avg_mth_eb, x= perecd, size = tot_asmts_mth)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "Mean Standardized Math Score",
       x = "Percent of ECD Students",
       cex.axis = 1.5) +
  theme_bw() +
  facet_wrap(~subgroup) + 
  guides(size = FALSE)
...
```{r}
# showing correlation between information theory index and relative diversity index

rs_hs_reg <- lm(rswhtblk ~ hswhtblk, data = seda_oi_cov)

summary(rs_hs_reg)

seda_oi_cov_clean <- seda_oi_cov %>% filter(!is.na(rswhtblk)) %>% filter(!is.na(hswhtblk))

correl = dplyr::summarise(seda_oi_cov_clean, correl = cor(rswhtblk, hswhtblk))
...
rswhtblk_test_all <- seda_oi_cov %>% 
  filter(subgroup == "all") %>% 
  ggplot(aes(y=mn_avg_mth_eb, x= rswhtblk, size = tot_asmts_mth)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "Mean Standardized Math Score",
       x = "White-Black Relative Diversity Index",
       cex.axis = 1.5) +
  theme_bw() +
   guides(size = FALSE)
...
# maybe don't need this, information index instead of relative diversity index

hswhtblk_test_all <- seda_oi_cov %>% 
  filter(subgroup == "all") %>% 
  ggplot(aes(y =mn_avg_mth_eb, x = hswhtblk, size = tot_asmts_mth)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "Mean Standardized Math Score",
       x = "White-Black Information Index",
       cex.axis = 1.5) +
  theme_bw() +
   guides(size = FALSE)
...
rswhthsp_test_all <- seda_oi_cov %>% 
  filter(subgroup == "all") %>% 
  ggplot(aes(y =mn_avg_mth_eb, x = rswhthsp, size = tot_asmts_mth)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "Mean Standardized Math Score",
       x = "White-Hispanic Relative Diversity Index",
       cex.axis = 1.5) +
  theme_bw() +
   guides(size = FALSE)
...
# WB diversity v test score separated by race (avg scores)
...
# WB diversity by race (avg learning rates)
rswhtblk_grd_by_race <- seda_oi_cov %>% 
  filter(subgroup == "asn" | subgroup == "blk" | subgroup == "hsp" | subgroup == "wht") %>% 
  ggplot(aes(y=mn_grd_mth_eb, x= rswhtblk, size = tot_asmts_mth)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "County Grade Slope of Mean Math Scores",
       x = "White-Black Relative Diversity Index",
  ...
  facet_wrap(~subgroup) +
   guides(size = FALSE)
ggsave("rswhtblk_grd_by_race.png")

# WB diversity by race (trends in avg test scores)
rswhtblk_coh_by_race <- seda_oi_cov %>% 
  filter(subgroup == "asn" | subgroup == "blk" | subgroup == "hsp" | subgroup == "wht") %>% 
  ggplot(aes(y=mn_coh_mth_eb, x= rswhtblk, size = tot_asmts_mth)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "County Cohort Slope of Mean Math Scores",
       x = "White-Black Relative Diversity Index",
  ...
  facet_wrap(~subgroup) +
   guides(size = FALSE)
ggsave("rswhtblk_coh_by_race.png")
...
```
```{r}
# WH diversity separated by race (avg scores)
...
# WH diversity separated by race (avg learned rates)
rswhthsp_grd_by_race <- seda_oi_cov %>% 
  filter(subgroup == "asn" | subgroup == "blk" | subgroup == "hsp" | subgroup == "wht") %>% 
  ggplot(aes(y=mn_grd_mth_eb, x= rswhthsp, size = tot_asmts_mth)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "County Grade Slope of Mean Math Scores",
       x = "White-Hispanic Relative Diversity Index",
...
ggsave("rswhthsp_grd_by_race.png")

# WH Diversity separated by race (trends in avg. test score)
rswhthsp_coh_by_race <- seda_oi_cov %>% 
  filter(subgroup == "asn" | subgroup == "blk" | subgroup == "hsp" | subgroup == "wht") %>% 
  ggplot(aes(y=mn_coh_mth_eb, x= rswhthsp, size = tot_asmts_mth)) +
  geom_point(alpha=.25) + 
  geom_smooth(method=lm, se= FALSE) +
  labs(y = "County Cohort Slope of Mean Math Scores",
       x = "White-Hispanic Relative Diversity Index",
...
ggsave("rswhthsp_coh_by_race.png")
```

```{r regressions}

# kfr regressions by each measure of diversity for math scores (maybe don't include if unnecessary)

kfr_math = lm(kfr_pooled_pooled_p25 ~ mn_avg_mth_ol, data = seda_oi_cov)
kfr_math_1 = lm(kfr_pooled_pooled_p25 ~ mn_avg_mth_ol + rswhtblk, data = seda_oi_cov)
kfr_math_2 = lm(kfr_pooled_pooled_p25 ~ mn_avg_mth_ol + rswhtblk + rswhthsp, data = seda_oi_cov)
kfr_math_3 = lm(kfr_pooled_pooled_p25 ~ mn_avg_mth_ol + rswhtblk + rswhthsp + rsecdnec, data = seda_oi_cov)

kfr_ela = lm(kfr_pooled_pooled_p25 ~ mn_avg_ela_ol, data = seda_oi_cov)
kfr_ela_1 = lm(kfr_pooled_pooled_p25 ~ mn_avg_ela_ol + rswhtblk, data = seda_oi_cov)
kfr_ela_2 = lm(kfr_pooled_pooled_p25 ~ mn_avg_ela_ol + rswhtblk + rswhthsp, data = seda_oi_cov)
kfr_ela_3 = lm(kfr_pooled_pooled_p25 ~ mn_avg_ela_ol + rswhtblk + rswhthsp + rsecdnec, data = seda_oi_cov)

stargazer(kfr_math, kfr_math_1, kfr_math_2, kfr_math_3, kfr_ela, kfr_ela_1, kfr_ela_2, kfr_ela_3,
          type="html", 
          dep.var.labels=c("Intergenerational Mobility"),
          column.labels = c("Math", "ELA"),
          covariate.labels=c("Standardized Mean Math Score", "Relative Diversity index between schools: White/Black", "Relative Diversity index between schools: White/Hispanic", "Relative Diversity index between schools: Economically disadvantaged/Non ECD","Standardized Mean ELA Score"),
          column.separate = c(4,4), out="kfr_div.html")
...
# adding in Chetty control variables that are not already included:
# residential segregation, income inequality (gini), social capital (crime_violent)
# racial_segregation + gini + crime_violent

kfr_avg_math_3 = lm(kfr_pooled_pooled_p25 ~ mn_avg_mth_ol + rswhtblk + rswhthsp + rsecdnec + sesavgall + perblk + perhsp + perecd + sesavgwhtblk + sesavgwhthsp + racial_segregation + gini + violent_crime_rate, data = seda_oi_cov)

x = lm(kfr_pooled_pooled_p25 ~ mn_avg_mth_ol + rswhtblk + rswhthsp, data = (seda_oi_cov %>% filter(subgroup == "wht")))
y = lm(kfr_pooled_pooled_p25 ~ mn_avg_mth_ol + rswhthsp, data = (seda_oi_cov %>% filter(subgroup == "wht")))
x1 = lm(kfr_pooled_pooled_p25 ~ mn_avg_mth_ol + rswhtblk + rswhthsp, data = (seda_oi_cov %>% filter(subgroup == "hsp")))
y1 = lm(kfr_pooled_pooled_p25 ~ mn_avg_mth_ol + rswhthsp, data = (seda_oi_cov %>% filter(subgroup == "hsp")))
x2 = lm(kfr_pooled_pooled_p25 ~ mn_avg_mth_ol + rswhtblk + rswhthsp, data = (seda_oi_cov %>% filter(subgroup == "blk")))
y2 = lm(kfr_pooled_pooled_p25 ~ mn_avg_mth_ol + rswhthsp, data = (seda_oi_cov %>% filter(subgroup == "blk")))
...
kfr_avg_ela_3 = lm(kfr_pooled_pooled_p25 ~ mn_avg_ela_ol + rswhtblk + rswhthsp + rsecdnec + sesavgall + perblk + perhsp + perecd + sesavgwhtblk + sesavgwhthsp + racial_segregation + gini + violent_crime_rate, data = seda_oi_cov)
...
stargazer(kfr_avg_math, kfr_avg_math_1, kfr_avg_math_2, kfr_avg_math_3, 
          kfr_avg_ela, kfr_avg_ela_1, kfr_avg_ela_2, kfr_avg_ela_3,
          type="html", 
          dep.var.labels=c("Intergenerational Mobility"),
          covariate.labels=c("Standardized Mean Math Score", "Standardized Mean ELA Score", "Relative Diversity index between schools: White/Black", "Relative Diversity index between schools: White/Hispanic", "Relative Diversity index between schools: Economically disadvantaged/Non ECD","SES Composite Score", "Percent Blacks in the Grade", "Percent Hispanic in the Grade", "Percent Economically Disadvantaged in the Grade", "White-Black Gap in SES Composite Score", "White-Hispanic Gap in SES Composite Score", "Racial Segregation", "Gini Coefficient", "Rate of Violent Crime"),
          column.labels = c("Without Controls", "With SES Composite", "With SEDA Controls", "With OI Controls", "Without Controls","With SES Composite", "With SEDA Controls", "With OI Controls"), out="kfr_avg_math_ela.html")

```

```{r}
# regressions for intergenerational mobility and average learning rate
# Control variables: + racial_segregation + gini + violent_crime_rate
# , "Racial Segregation", "Gini Coefficient", "Rate of Violent Crime"
...
kfr_grd_math_3 = lm(kfr_pooled_pooled_p25 ~ mn_grd_mth_ol + rswhtblk+ rswhthsp + rsecdnec + sesavgall + perblk + perhsp + perecd + sesavgwhtblk + sesavgwhthsp + racial_segregation + gini + violent_crime_rate, data = seda_oi_cov)
...
kfr_grd_ela_3 = lm(kfr_pooled_pooled_p25 ~ mn_grd_ela_ol + rswhtblk +rswhthsp + rsecdnec + sesavgall + perblk + perhsp + perecd + sesavgwhtblk + sesavgwhthsp + racial_segregation + gini + violent_crime_rate, data = seda_oi_cov)
...
stargazer(kfr_grd_math, kfr_grd_math_1, kfr_grd_math_2, kfr_grd_math_3, kfr_grd_ela, kfr_grd_ela_1, kfr_grd_ela_2, kfr_grd_ela_3,
          type="html", 
          dep.var.labels=c("Intergenerational Mobility"),
          covariate.labels=c("County Grade Slope of Mean Achievement in Math", "County Grade Slope for Mean Achievement in ELA", "Relative Diversity index between schools: White/Black", "Relative Diversity index between schools: White/Hispanic", "Relative Diversity index between schools: Economically disadvantaged/Non ECD","SES Composite Score", "Percent Blacks in the Grade", "Percent Hispanic in the Grade", "Percent Economically Disadvantaged in the Grade", "White-Black Gap in SES Composite Score", "White-Hispanic Gap in SES Composite Score", "Racial Segregation", "Gini Coefficient", "Rate of Violent Crime"),
          column.labels = c("Without Controls", "With SES Composite", "With SEDA Controls", "With OI Controls", "Without Controls","With SES Composite", "With SEDA Controls", "With OI Controls"), out="kfr_grd_math_ela.html")

```

```{r}
# models for trends in average test scores
# Control variables: + racial_segregation + gini + violent_crime_rate
# , "Racial Segregation", "Gini Coefficient", "Rate of Violent Crime"
...
kfr_coh_math_3 = lm(kfr_pooled_pooled_p25 ~ mn_coh_mth_ol + rswhtblk + rswhthsp + rsecdnec + sesavgall + perblk + perhsp + perecd + sesavgwhtblk + sesavgwhthsp + racial_segregation + gini + violent_crime_rate, data = seda_oi_cov)
...
kfr_coh_ela_3 = lm(kfr_pooled_pooled_p25 ~ mn_coh_ela_ol + rswhtblk + rswhthsp + rsecdnec + sesavgall + perblk + perhsp + perecd + sesavgwhtblk + sesavgwhthsp + racial_segregation + gini + violent_crime_rate, data = seda_oi_cov)
...
stargazer(kfr_coh_math, kfr_coh_math_1, kfr_coh_math_2, kfr_coh_math_3, kfr_coh_ela, kfr_coh_ela_1, kfr_coh_ela_2, kfr_coh_ela_3,
          type="html", 
          dep.var.labels=c("Intergenerational Mobility"),
          covariate.labels=c("County Cohort Slope of Mean Achievement in Math", "County Cohort Slope of Mean Achievement in ELA", "Relative Diversity index between schools: White/Black", "Relative Diversity index between schools: White/Hispanic", "Relative Diversity index between schools: Economically disadvantaged/Non ECD","SES Composite Score", "Percent Blacks in the Grade", "Percent Hispanic in the Grade", "Percent Economically Disadvantaged in the Grade", "White-Black Gap in SES Composite Score", "White-Hispanic Gap in SES Composite Score", "Racial Segregation", "Gini Coefficient", "Rate of Violent Crime"),
          column.labels = c("Without Controls", "With SES Composite", "With SEDA Controls", "With OI Controls", "Without Controls","With SES Composite", "With SEDA Controls", "With OI Controls"), out="kfr_coh_math_ela.html")
```
...
# with controls added
div_score_cont <- lm(mn_avg_mth_ol~ rswhtblk + rswhthsp + rsecdnec + sesavgall, data = seda_oi_cov)
div_score_cont_1 <- lm(mn_avg_mth_ol~ rswhtblk + rswhthsp + rsecdnec + sesavgall + perblk + perhsp + perecd + sesavgwhtblk + sesavgwhthsp, data = seda_oi_cov)
...
div_score_ela_cont <-lm(mn_avg_ela_ol~ rswhtblk + rswhthsp + rsecdnec + sesavgall, data = seda_oi_cov)
div_score_ela_cont_1 <-lm(mn_avg_ela_ol~ rswhtblk + rswhthsp + rsecdnec + sesavgall+ perblk + perhsp + perecd + sesavgwhtblk + sesavgwhthsp, data = seda_oi_cov)
...
stargazer(div_score, div_score_cont, div_score_cont_1, div_score_ela, div_score_ela_cont, div_score_ela_cont_1, 
          type="html", 
          ...
          column.labels = c("Without Controls", "With SES Composite", "With SEDA Controls", "Without Controls", "With SES Composite", "With SEDA Controls"), out="div_score_subject.html")
```

```{r}
# regressions on the correlations between diversity within school and grade slope of scores 
# took out "sesavgall" and only included it in the regressions with controls 
div_grd_score <- lm(mn_grd_mth_ol ~ rswhtblk + rswhthsp + rsecdnec, data = seda_oi_cov)

summary(div_grd_score)

div_grd_score_ela <-lm(mn_grd_ela_ol~ rswhtblk + rswhthsp + rsecdnec, data = seda_oi_cov)
summary(div_grd_score_ela) 

# with controls added
div_grd_score_cont <- lm(mn_grd_mth_ol~ rswhtblk + rswhthsp + rsecdnec + sesavgall, data = seda_oi_cov)
div_grd_score_cont_1 <- lm(mn_grd_mth_ol~ rswhtblk + rswhthsp + rsecdnec + sesavgall + perblk + perhsp + perecd + sesavgwhtblk + sesavgwhthsp, data = seda_oi_cov)

summary(div_grd_score_cont)

div_grd_score_ela_cont <-lm(mn_grd_ela_ol~ rswhtblk + rswhthsp + rsecdnec + sesavgall, data = seda_oi_cov)
div_grd_score_ela_cont_1 <-lm(mn_grd_ela_ol~ rswhtblk + rswhthsp + rsecdnec + sesavgall+ perblk + perhsp + perecd + sesavgwhtblk + sesavgwhthsp, data = seda_oi_cov)

summary(div_grd_score_ela_cont) 

```

```{r}
# regression table of grade slopes and diversity

stargazer(div_grd_score, div_grd_score_cont, div_grd_score_cont_1, div_grd_score_ela, div_grd_score_ela_cont, div_grd_score_ela_cont_1,
          type="html", 
          dep.var.labels=c("County Grade Slope of Math Scores", "County Grade Slope of ELA Scores"),
          covariate.labels=c("Relative Diversity index between schools: White/Black", "Relative Diversity index between schools: White/Hispanic", "Relative Diversity index between schools: Economically disadvantaged/Non ECD","SES Composite Score", "Percent Blacks in the Grade", "Percent Hispanic in the Grade", "Percent Economically Disadvantaged in the Grade", "White-Black Gap in SES Composite Score", "White-Hispanic Gap in SES Composite Score"),
          column.labels = c("Without Controls",  "With SES Composite", "With SEDA Controls", "Without Controls",  "With SES Composite", "With SEDA Controls"), out="div_grd_score_subject.html")

```


```{r}
# regressions on the correlations between diversity within school and county cohort slopes of scores 
# took out "sesavgall" and only included it in the regressions with controls 
div_coh_score <- lm(mn_coh_mth_ol ~ rswhtblk + rswhthsp + rsecdnec, data = seda_oi_cov)

summary(div_coh_score)

div_coh_score_ela <-lm(mn_coh_ela_ol~ rswhtblk + rswhthsp + rsecdnec, data = seda_oi_cov)
summary(div_coh_score_ela) 

# with controls added
div_coh_score_cont <- lm(mn_coh_mth_ol~ rswhtblk + rswhthsp + rsecdnec + sesavgall, data = seda_oi_cov)
div_coh_score_cont_1 <- lm(mn_coh_mth_ol~ rswhtblk + rswhthsp + rsecdnec + sesavgall + perblk + perhsp + perecd + sesavgwhtblk + sesavgwhthsp, data = seda_oi_cov)

summary(div_coh_score_cont)
div_coh_score_ela_cont <-lm(mn_coh_ela_ol~ rswhtblk + rswhthsp + rsecdnec + sesavgall, data = seda_oi_cov)
div_coh_score_ela_cont_1 <-lm(mn_coh_ela_ol~ rswhtblk + rswhthsp + rsecdnec + sesavgall+ perblk + perhsp + perecd + sesavgwhtblk + sesavgwhthsp, data = seda_oi_cov)

summary(div_coh_score_ela_cont) 
```

```{r}
# regression table of cohort slopes and diversity

stargazer(div_coh_score, div_coh_score_cont, div_coh_score_cont_1, div_coh_score_ela, div_coh_score_ela_cont,div_coh_score_ela_cont_1, 
          type="html", 
          dep.var.labels=c("County Cohort Slope of Math Scores", "County Cohort Slope of ELA Scores"),
          covariate.labels=c("Relative Diversity index between schools: White/Black", "Relative Diversity index between schools: White/Hispanic", "Relative Diversity index between schools: Economically disadvantaged/Non ECD","SES Composite Score", "Percent Blacks in the Grade", "Percent Hispanic in the Grade", "Percent Economically Disadvantaged in the Grade", "White-Black Gap in SES Composite Score", "White-Hispanic Gap in SES Composite Score"),
          column.labels = c("Without Controls", "With SES Composite", "With SEDA Controls", "Without Controls", "With SES Composite", "With SEDA Controls"), out="div_coh_score_subject.html")
```
